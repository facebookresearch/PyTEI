...
class Injector:
    ...
    @classmethod
    def _error_map_generate(
        cls, injectee_shape: tuple, dtype_bitwidth: int, device: torch.device, p: float
    ) -> torch.Tensor:
        ...
        error_map = (
            ...
            * torch.ones(
            ...
            dims=(-1,)
        ).expand((*injectee_shape, dtype_bitwidth))
        filter = (
            p
        ...
        for param_name, param in model.named_parameters():
            for each_param in self.targets:
                if each_param == param_name:
                    ...
                    self._error_maps[param_name], self._error_count[param_name] = (
                        Injector._error_map_generate(
    ...
    def _assign_targets(self) -> None:
        ...
        with open(self.target_path, "r") as f:
            ...
            ]  # Use semicolon (`;`) to comment out a parameter.
    ...
    def _perform_mitigation(self, model) -> None:
        ...
        if self.mitigation != None:
            if self.mitigation == "SBP":
                self._error_maps = Defender.sbp(
                    self._error_maps,
                    protected_bits=[31, 30, 29, 28, 27, 26, 25, 24, 23],
                )
            if self.mitigation == "clip":
                # Defender.activation_clipping(model, min = -6, max = 6) # This is referred from the ReLU6
                Defender.activation_clipping(
                    model, min=-100, max=100
                )  # This is a very loose clipping
